...
def serve(artifact_dir: str = "artifacts", host: str = "0.0.0.0", port: int = 8080):
    ...
    print(f"CLCRec Lab API  http://{host}:{port}", flush=True)
    print("  GET  /v1/health", flush=True)
    print("  POST /v1/similar     {item_id, k, pool}", flush=True)
    print("  POST /v1/recommend   {user_id|liked_ids, k, pool}", flush=True)
    print("  POST /v1/new_item    {title, genres, year, k}", flush=True)
    httpd.serve_forever()
